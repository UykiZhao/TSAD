...
# 导入消融实验模型
from models.DualScopeNet_Ablation import Model as DualScopeNet_Ablation
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='DualScopeNet Ablation Study')
    ...
    parser.add_argument('--model', type=str, default='DualScopeNet_Ablation',
                        help='model name')

    # ablation specific
    parser.add_argument('--ablation_mode', type=str, default='full',
                        choices=['full', 'no_pki', 'no_mstc', 'no_adaptive_fusion', 
                                'no_anomaly_fusion', 'periodic_only', 'aperiodic_only'],
                        help='ablation mode')
    ...
    # 将消融模式添加到args
    args.model = 'DualScopeNet_Ablation'
    ...
    # 创建自定义的实验类
    class Exp_Anomaly_Detection_Ablation(Exp_Anomaly_Detection):
        def _build_model(self):
            # 使用消融实验模型
            model = DualScopeNet_Ablation(self.args).float()
            if self.args.use_multi_gpu and self.args.use_gpu:
                model = nn.DataParallel(model, device_ids=self.args.device_ids)
            return model

    Exp = Exp_Anomaly_Detection_Ablation

    if args.is_training:
        for ii in range(args.itr):
            # setting record of experiments
            exp = Exp(args)  # set experiments
            setting = 'ablation_{}_{}_{}_{}_ft{}_sl{}_ll{}_pl{}_dm{}_nh{}_el{}_dl{}_df{}_fc{}_eb{}_dt{}_{}_{}'.format(
                args.ablation_mode,
                args.model_id,
                args.model,
                args.data,
